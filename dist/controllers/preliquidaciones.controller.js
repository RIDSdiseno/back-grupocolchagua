...
exports.eliminarPreLiquidacion = exports.aprobarPreLiquidacion = exports.actualizarPreLiquidacion = exports.obtenerPreLiquidacion = exports.generarPreLiquidaciones = exports.listarPreLiquidaciones = void 0;
...
function validarMesAnio(mes, anio) {
...
function obtenerEstadoAsistencia(fecha, estado) {
...
const listarPreLiquidaciones = async (req, res) => {
    try {
        ...
        const preliquidaciones = await prisma_1.prisma.preLiquidacion.findMany({
            where,
            include,
...
exports.listarPreLiquidaciones = listarPreLiquidaciones;
const generarPreLiquidaciones = async (req, res) => {
    try {
        ...
        for (const item of porTrabajador.values()) {
            ...
            const incidencias = await prisma_1.prisma.incidenciaAsistencia.findMany({
                where: {
                    trabajadorId: item.trabajadorId,
                    empresaId: item.empresaId,
                    sucursalId: item.sucursalId,
            ...
            const totalHaberes = redondear(sueldoBase +
                montoHorasExtras +
                colacion +
                movilizacion +
                bonoTurnoNocturno +
            ...
            const preliquidacion = await prisma_1.prisma.preLiquidacion.upsert({
                where: {
...
exports.generarPreLiquidaciones = generarPreLiquidaciones;
const obtenerPreLiquidacion = async (req, res) => {
    try {
        ...
        const preliquidacion = await prisma_1.prisma.preLiquidacion.findUnique({
            where: { id },
            include,
        });
        if (!preliquidacion) {
...
exports.obtenerPreLiquidacion = obtenerPreLiquidacion;
const actualizarPreLiquidacion = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({
        ...
        const existente = await prisma_1.prisma.preLiquidacion.findUnique({
            where: { id },
        });
        if (!existente) {
            return res.status(404).json({
        ...
        const totalHaberes = redondear(Number(base.montoDiasTrabajados) +
            ...
            Number(base.difLiq));
        const totalDescuentos = redondear(Number(base.totalDescuentos) - Number(existente.anticipo) + Number(base.anticipo));
        ...
        const preliquidacion = await prisma_1.prisma.preLiquidacion.update({
            where: { id },
            data,
            include,
        });
...
exports.actualizarPreLiquidacion = actualizarPreLiquidacion;
const aprobarPreLiquidacion = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({
...
exports.aprobarPreLiquidacion = aprobarPreLiquidacion;
const eliminarPreLiquidacion = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({
        ...
        await prisma_1.prisma.preLiquidacion.delete({
            where: { id },
        });
        return res.json({
            ok: true,
...
exports.eliminarPreLiquidacion = eliminarPreLiquidacion;
