...
import type {
  ...
  TalanaMarca,
  TalanaMarcaPersona,
  TrabajadorConMarcacionesTalana,
} from "../../types/talana/talana.marcas.types";

import { formatearRutParaTalana } from "../../utils/talana/talana.rut";

function construirNombreCompleto(
  persona: TalanaMarcaPersona,
): string {
  return [
    persona.nombre,
...
async function esperarEntreSolicitudes(): Promise<void> {
  const delayMs = Number(
    process.env.TALANA_REQUEST_DELAY_MS ?? 0,
  );
...
export async function probarConexionTalana() {
  const response: AxiosResponse<
    TalanaPaginatedResponse<unknown>
  > = await talanaApi.get<
    TalanaPaginatedResponse<unknown>
  >("/mark/", {
    params: {
...
export async function listarTrabajadoresQueMarcanTalana(
  desde: string,
  hasta: string,
  empresaId: number,
): Promise<ResultadoTrabajadoresConMarcacionesTalana> {
  validarEmpresaTalana(empresaId);

  const pageSize = Number(
    process.env.TALANA_MARK_PAGE_SIZE ?? 100,
  );

  const limitePaginas = Number(
    process.env.TALANA_MARK_MAX_PAGES ?? 5000,
  );
  ...
  if (
    ...
    throw new Error(
      "TALANA_MARK_PAGE_SIZE debe ser un entero mayor que cero",
    );
  }

  if (
    !Number.isInteger(limitePaginas) ||
    limitePaginas <= 0
  ) {
    throw new Error(
      "TALANA_MARK_MAX_PAGES debe ser un entero mayor que cero",
    );
  }
  ...
  while (pagina <= limitePaginas) {
    console.log(
      `[Talana] Marcaciones empresa=${empresaId}, ` +
        `desde=${desde}, hasta=${hasta}, página=${pagina}`,
    );

    const response: AxiosResponse<
      TalanaPaginatedResponse<TalanaMarca>
    > = await talanaApi.get<
      TalanaPaginatedResponse<TalanaMarca>
    >("/mark/", {
      params: {
    ...
    const marcas: TalanaMarca[] = Array.isArray(
      response.data.results,
    )
      ? response.data.results
      : [];
    ...
    totalMarcaciones = Number(
      response.data.count ?? marcas.length,
    );
    ...
    for (const marca of marcas) {
      ...
      if (!existente) {
        trabajadores.set(clave, {
          ...
          rut: formatearRutParaTalana(
            marca.person.rut,
          ),

          nombre: marca.person.nombre,
          apellidoPaterno:
            marca.person.apellidoPaterno,
          apellidoMaterno:
            marca.person.apellidoMaterno,
          ...
          oficinas:
            typeof marca.office === "number"
              ? [marca.office]
              : [],

          metodosMarcacion:
            typeof marca.markingMethod ===
              "string" &&
      ...
      const timestampMarca = new Date(
        marca.TS,
      ).getTime();

      const timestampPrimera = new Date(
        existente.primeraMarcacion,
      ).getTime();

      const timestampUltima = new Date(
        existente.ultimaMarcacion,
      ).getTime();

      if (
        Number.isFinite(timestampMarca) &&
      ...
      const metodo =
        typeof marca.markingMethod === "string"
          ? marca.markingMethod.trim()
          : "";

      if (
        metodo &&
        !existente.metodosMarcacion.includes(
          metodo,
        )
      ) {
    ...
    console.log(
      `[Talana] Página ${pagina}: ` +
        `${marcas.length} marcas, ` +
        `${trabajadores.size} trabajadores únicos`,
    );

    if (
      marcacionesProcesadas >= totalMarcaciones ||
  ...
  if (pagina > limitePaginas) {
    throw new Error(
      `Se superó el límite de ${limitePaginas} páginas de marcaciones`,
    );
  }
