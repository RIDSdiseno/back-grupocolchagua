// src/controllers/oportunidad.controller.ts
import { Response } from "express";
import { Prisma } from "@prisma/client";
import { prisma } from "../lib/prisma";
import type { AuthRequest } from "../middlewares/auth.middleware";
import type {
  DatosEtapaContratados,
  DatosEtapaDocumentacion,
  DocumentoRequeridoOportunidad,
  EstadoOportunidad,
  FuentePostulacionExterna,
  OportunidadContratadosMotivoNoIngreso,
  OportunidadReclutamiento,
} from "@prisma/client";

type JornadaOportunidad = "FULL_TIME" | "PART_TIME" | "AMBAS";
type EstadoBusquedaOportunidad = "ACTIVA" | "PAUSADA" | "CERRADA" | "CANCELADA";
type PrioridadOportunidad = "ALTA" | "MEDIA" | "BAJA";
type TipoMovimientoOportunidad =
  | "CREACION"
  | "EDICION_GENERAL"
  ...
  | "MARCADA_PERDIDA"
  | "POSTERGADA";
type EstadoFinalProceso = "VACANTE_CUBIERTA" | "VACANTE_PARCIALMENTE_CUBIERTA" | "VACANTE_NO_CUBIERTA";

const JORNADAS_OPORTUNIDAD: JornadaOportunidad[] = ["FULL_TIME", "PART_TIME", "AMBAS"];
const ESTADOS_BUSQUEDA: EstadoBusquedaOportunidad[] = ["ACTIVA", "PAUSADA", "CERRADA", "CANCELADA"];
const PRIORIDADES_OPORTUNIDAD: PrioridadOportunidad[] = ["ALTA", "MEDIA", "BAJA"];
...
const parseEnteroNoNegativo = (valor: unknown): number | null => {
...
const parseBooleanOpcional = (valor: unknown): boolean | null => {
  if (valor === undefined || valor === null || String(valor).trim() === "") return null;
  if (valor === true || valor === "true" || valor === "SI") return true;
  if (valor === false || valor === "false" || valor === "NO") return false;
  return null;
};

const normalizarNombreFuente = (valor: string): string =>
  valor.trim().toLowerCase().replace(/\s+/g, " ");

const normalizarNombreDocumento = (valor: string): string =>
  valor.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, " ");

const normalizarMotivoNoAptitud = (motivos: string[], motivoPersonalizado: string | null): string => {
...
const normalizarMotivoNoIngreso = (motivo: string, motivoOtro: string | null): string =>
  motivo === "Otro"
    ? `otro::${(motivoOtro ?? "").trim().toLowerCase().replace(/\s+/g, " ")}`
    : motivo.trim().toLowerCase();

const calcularEstadoFinalProceso = (personasContratadas: number, personasNecesarias: number): EstadoFinalProceso => {
  if (personasContratadas >= personasNecesarias) return "VACANTE_CUBIERTA";
  if (personasContratadas > 0) return "VACANTE_PARCIALMENTE_CUBIERTA";
  return "VACANTE_NO_CUBIERTA";
};

const calcularPostulantesOtros = (fuentes: { cantidad: number }[]): number =>
  fuentes.reduce((acc, f) => acc + f.cantidad, 0);
...
const registrarHistorial = async (params: {
  ...
  usuarioId?: number;
  detalle?: string | null;
  estadoAnterior?: EstadoOportunidad | null;
  estadoNuevo?: EstadoOportunidad | null;
}) => {
  if (!params.usuarioId) return;

  await prisma.oportunidadHistorial.create({
    data: {
...
type OportunidadConRelaciones = OportunidadReclutamiento & {
  Empresa: { id: number; nombre: string };
  Cargo: { id: number; nombre: string };
  Sucursal: { id: number; nombre: string; direccion: string | null; comuna: string | null; ciudad: string | null } | null;
  ReclutadorResponsable: { id: number; nombre: string };
};

const serializarDatosDocumentacion = (
  oportunidad: OportunidadReclutamiento & {
    Entrevista?: { aptos: number } | null;
    Documentacion?: DatosEtapaDocumentacion | null;
...
const serializarDatosContratados = (
  oportunidad: OportunidadReclutamiento & {
    Entrevista?: { aptos: number } | null;
    Documentacion?: { documentacionCompleta: number } | null;
    Contratados?: (DatosEtapaContratados & { MotivosNoIngreso: OportunidadContratadosMotivoNoIngreso[] }) | null;
  }
) => {
...
const estadoVisibleOportunidad = (estado: EstadoOportunidad): EstadoOportunidad =>
  estado === "SELECCIONADOS" ? "DOCUMENTACION" : estado;

const serializarOportunidad = (oportunidad: OportunidadConRelaciones) => ({
...
const construirFiltrosOportunidades = (query: Record<string, unknown>): Prisma.OportunidadReclutamientoWhereInput => {
  const where: Prisma.OportunidadReclutamientoWhereInput = {};

  if (query.empresaId) where.empresaId = Number(query.empresaId);
  if (query.cargoId) where.cargoId = Number(query.cargoId);
  if (query.reclutadorId) where.reclutadorResponsableId = Number(query.reclutadorId);

  if (query.estado) {
    where.estado = String(query.estado) as EstadoOportunidad;
  }
...
export const listarOportunidades = async (req: AuthRequest, res: Response) => {
  try {
    const where = construirFiltrosOportunidades(req.query as Record<string, unknown>);

    const oportunidades = await prisma.oportunidadReclutamiento.findMany({
      where,
    ...
    return res.json({
      ...
      oportunidades: oportunidades.map((o) => {
        ...
        return {
          ...serializarOportunidad(o as OportunidadConRelaciones),
          postulantesWeb,
          postulantesOtros,
...
export const obtenerOportunidad = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const oportunidad = await prisma.oportunidadReclutamiento.findUnique({
      where: { id },
      relationLoadStrategy: "join",
      include: {
        ...INCLUDE_OPORTUNIDAD,
    ...
    return res.json({
      ok: true,
      oportunidad: {
        ...serializarOportunidad(oportunidad as OportunidadConRelaciones),
        etapaNuevos: {
...
const validarDatosGenerales = async (body: Record<string, unknown>) => {
  ...
  if (!JORNADAS_OPORTUNIDAD.includes(String(jornada) as JornadaOportunidad)) {
  ...
  const [empresa, cargo, reclutador] = await Promise.all([
    ...
    prisma.cargo.findUnique({ where: { id: cargoIdNum } }),
    prisma.usuario.findUnique({ where: { id: reclutadorIdNum } }),
  ]);

  if (!empresa) return { error: "La empresa seleccionada no existe" };
  if (!cargo) return { error: "El cargo seleccionado no existe" };
  if (!reclutador) return { error: "El reclutador responsable seleccionado no existe" };

  let sucursalIdNum: number | null = null;
  ...
  return {
    data: {
      ...
      jornada: String(jornada) as JornadaOportunidad,
...
export const crearOportunidad = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const oportunidad = await prisma.oportunidadReclutamiento.create({
    ...
    return res.status(201).json({
      ...
      oportunidad: serializarOportunidad(oportunidad as OportunidadConRelaciones),
    });
  } catch (error) {
...
export const actualizarOportunidad = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existente = await prisma.oportunidadReclutamiento.findUnique({ where: { id } });
    ...
    const oportunidad = await prisma.oportunidadReclutamiento.update({
      where: { id },
      data: validacion.data,
      include: INCLUDE_OPORTUNIDAD,
    });
    ...
    if (existente.reclutadorResponsableId !== oportunidad.reclutadorResponsableId) {
      const reclutadorAnterior = await prisma.usuario.findUnique({ where: { id: existente.reclutadorResponsableId } });
      const reclutadorNuevo = (oportunidad as OportunidadConRelaciones).ReclutadorResponsable.nombre;
...
const ESTADOS_OPORTUNIDAD_VALIDOS: EstadoOportunidad[] = [
  "NUEVOS",
  "PRESELECCION",
  "ENTREVISTA",
  "DOCUMENTACION",
...
export const actualizarEstadoOportunidad = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    if (!ESTADOS_OPORTUNIDAD_VALIDOS.includes(String(estado) as EstadoOportunidad)) {
    ...
    const estadoNuevo = String(estado) as EstadoOportunidad;
...
export const postergarOportunidad = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
...
export const actualizarEtapaNuevos = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    let empleoIdNum: number | null = null;

    if (empleoId) {
      empleoIdNum = Number(empleoId);
      const empleo = await prisma.empleo.findUnique({ where: { id: empleoIdNum } });
    ...
    if (estadoBusqueda && !ESTADOS_BUSQUEDA.includes(String(estadoBusqueda) as EstadoBusquedaOportunidad)) {
      return res.status(400).json({ ok: false, message: "Estado de búsqueda inválido", estadosPermitidos: ESTADOS_BUSQUEDA });
    }

    if (prioridad && !PRIORIDADES_OPORTUNIDAD.includes(String(prioridad) as PrioridadOportunidad)) {
    ...
    const oportunidad = await prisma.oportunidadReclutamiento.update({
      ...
      data: {
        ...
        estadoBusqueda: estadoBusqueda ? (String(estadoBusqueda) as EstadoBusquedaOportunidad) : null,
        prioridad: prioridad ? (String(prioridad) as PrioridadOportunidad) : null,
...
export const actualizarEtapaPreseleccion = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    const existente = await prisma.oportunidadReclutamiento.findUnique({
      where: { id },
      include: {
        Empleo: { select: { _count: { select: { postulaciones: true } } } },
        FuentesExternas: { select: { cantidad: true } },
...
export const actualizarEtapaEntrevista = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    const motivosNoAptitud: {
      motivos: string[];
      motivoPersonalizado: string | null;
    ...
    for (const fila of motivosNoAptitudRaw) {
      const motivosLimpios: string[] = Array.isArray(fila?.motivos)
        ? fila.motivos
            .map((m: unknown) => limpiarTexto(m))
            .filter((m: string | null): m is string => !!m)
        : [];
      const motivosFila = Array.from(new Set<string>(motivosLimpios));
    ...
    let responsableEntrevistaId: number | null = null;
    if (req.body.responsableEntrevistaId) {
      ...
      const responsable = await prisma.usuario.findUnique({ where: { id: responsableEntrevistaId } });
      if (!responsable) {
...
const buscarOportunidadParaDocumentacion = async (id: number) =>
  prisma.oportunidadReclutamiento.findUnique({
    where: { id },
    include: {
...
const validarEtapaDocumentacion = (estado: EstadoOportunidad) =>
  estado === "DOCUMENTACION" || estado === "CONTRATADOS";

export const obtenerEtapaDocumentacion = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) return res.status(400).json({ ok: false, message: "ID de oportunidad invÃ¡lido" });

    const oportunidad = await buscarOportunidadParaDocumentacion(id);
    if (!oportunidad) return res.status(404).json({ ok: false, message: "Oportunidad no encontrada" });
...
export const actualizarEtapaDocumentacion = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) return res.status(400).json({ ok: false, message: "ID de oportunidad invÃ¡lido" });
    ...
    await prisma.datosEtapaDocumentacion.upsert({
      where: { oportunidadId: id },
      create: {
        oportunidadId: id,
...
const buscarOportunidadParaContratados = async (id: number) =>
  prisma.oportunidadReclutamiento.findUnique({
    where: { id },
    include: {
      Empresa: { select: { nombre: true } },
...
const validarEtapaContratados = (estado: EstadoOportunidad) => estado === "CONTRATADOS";

export const obtenerEtapaContratados = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) return res.status(400).json({ ok: false, message: "ID de oportunidad inválido" });
...
export const actualizarEtapaContratados = async (req: AuthRequest, res: Response) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) return res.status(400).json({ ok: false, message: "ID de oportunidad inválido" });
...
const serializarDocumentoRequerido = (documento: DocumentoRequeridoOportunidad) => ({
  id: String(documento.id),
  nombre: documento.nombre,
  tipoDocumento: documento.tipoDocumento,
  obligatorio: documento.obligatorio,
...
const resolverDocumentoRequeridoPayload = (body: Record<string, unknown>) => {
  ...
  if (!nombre) return { error: "Debe indicar el nombre del documento cuando selecciona Otro" };
...
export const crearDocumentoRequerido = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    if (Number.isNaN(oportunidadId)) return res.status(400).json({ ok: false, message: "ID de oportunidad invÃ¡lido" });
    ...
    if (payload.error || !payload.data) return res.status(400).json({ ok: false, message: payload.error });
    ...
    const documento = await prisma.documentoRequeridoOportunidad.create({
      data: {
        oportunidadId,
        ...payload.data,
      },
...
export const actualizarDocumentoRequerido = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    const documentoId = Number(req.params.documentoId);
    if (Number.isNaN(oportunidadId) || Number.isNaN(documentoId)) return res.status(400).json({ ok: false, message: "ID invÃ¡lido" });

    const existente = await prisma.documentoRequeridoOportunidad.findUnique({ where: { id: documentoId } });
    ...
    const documento = await prisma.documentoRequeridoOportunidad.update({
...
export const eliminarDocumentoRequerido = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    const documentoId = Number(req.params.documentoId);
    if (Number.isNaN(oportunidadId) || Number.isNaN(documentoId)) return res.status(400).json({ ok: false, message: "ID invÃ¡lido" });
    ...
    await prisma.documentoRequeridoOportunidad.delete({ where: { id: documentoId } });
...
const serializarFuente = (fuente: FuentePostulacionExterna) => ({
  id: String(fuente.id),
  nombre: fuente.nombre,
  cantidad: fuente.cantidad,
});

export const crearFuenteExterna = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    ...
    const oportunidad = await prisma.oportunidadReclutamiento.findUnique({ where: { id: oportunidadId } });
    ...
    const existente = await prisma.fuentePostulacionExterna.findUnique({
    ...
    const fuente = await prisma.fuentePostulacionExterna.create({
...
export const actualizarFuenteExterna = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    ...
    const fuente = await prisma.fuentePostulacionExterna.findUnique({ where: { id: fuenteId } });
    ...
    const duplicada = await prisma.fuentePostulacionExterna.findUnique({
      where: { oportunidadId_nombreNormalizado: { oportunidadId, nombreNormalizado } },
    });
    ...
    const actualizada = await prisma.fuentePostulacionExterna.update({
      where: { id: fuenteId },
...
export const eliminarFuenteExterna = async (req: AuthRequest, res: Response) => {
  try {
    const oportunidadId = Number(req.params.id);
    const fuenteId = Number(req.params.fuenteId);
    ...
    await prisma.fuentePostulacionExterna.delete({ where: { id: fuenteId } });
