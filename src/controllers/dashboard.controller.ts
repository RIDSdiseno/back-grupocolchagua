...
import type { EstadoNegociacionComercial, EstadoOportunidad, EtapaOportunidadComercial } from "@prisma/client";

const ESTADOS_FUNNEL: EstadoOportunidad[] = [
  "NUEVOS",
  "PRESELECCION",
  "ENTREVISTA",
  "SELECCIONADOS",
...
const ETAPAS_FUNNEL_COMERCIAL: EtapaOportunidadComercial[] = [
  "PROSPECTO",
  "CONTACTADO",
  "PROPUESTA",
  "NEGOCIACION",
...
const ESTADOS_NEGOCIACION_DASHBOARD: EstadoNegociacionComercial[] = [
  "EN_NEGOCIACION",
  "SOLICITA_CAMBIOS",
  "SOLICITA_REBAJA",
  "EN_EVALUACION",
...
const redondear1 = (valor: number): number => Math.round(valor * 10) / 10;

const promedio = (valores: number[]): number | null =>
  valores.length ? redondear1(valores.reduce((acc, v) => acc + v, 0) / valores.length) : null;

const diasEntre = (inicio: Date, fin: Date): number => Math.round((fin.getTime() - inicio.getTime()) / (1000 * 60 * 60 * 24));

const construirFiltros = (query: Record<string, unknown>): Prisma.OportunidadReclutamientoWhereInput => {
  ...
  if (query.oportunidadId) where.id = Number(query.oportunidadId);
...
const calcularTotalPostulantes = (o: { Empleo: { _count: { postulaciones: number } } | null; FuentesExternas: { cantidad: number }[] }): number => {
...
const principalDeConteo = (conteo: Map<string, number>): Principal | null => {
  let principal: Principal | null = null;
  for (const [motivo, cantidad] of conteo) {
    if (!principal || cantidad > principal.cantidad) principal = { motivo, cantidad };
  }
  return principal;
};

const todosDeConteo = (conteo: Map<string, number>): Principal[] =>
  Array.from(conteo.entries())
    .map(([motivo, cantidad]) => ({ motivo, cantidad }))
    .sort((a, b) => b.cantidad - a.cantidad);

export const obtenerDashboardReclutamiento = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const vacantesPendientes = oportunidades.reduce(
      (acc, o) => acc + Math.max(o.cantidadNecesaria - (o.Contratados?.personasContratadas ?? 0), 0),
      0
    );

    const tiemposContratacion = oportunidades
      .filter((o) => o.Contratados?.fechaCierreProceso)
      .map((o) => diasEntre(o.fechaInicio, o.Contratados!.fechaCierreProceso as Date));
    ...
    type AcumuladorGrupo = {
      nombre: string;
      empresas: Set<number>;
      oportunidades: number;
      oportunidadesActivas: number;
      personasSolicitadas: number;
      personasContratadas: number;
      vacantesPendientes: number;
      tiempos: number[];
    };

    const porReclutador = new Map<number, AcumuladorGrupo>();
    const porEmpresa = new Map<number, AcumuladorGrupo>();
    ...
    for (const o of oportunidades) {
      ...
      if (o.estado !== "CONTRATADOS") grupoReclutador.oportunidadesActivas += 1;
      ...
      if (tiempo !== null) grupoReclutador.tiempos.push(tiempo);
    ...
    const oportunidadesCubiertasCompletamente = oportunidades.filter(
      (o) => (o.Contratados?.personasContratadas ?? 0) >= o.cantidadNecesaria
    ).length;
    ...
    const conteoMotivoDescarte = new Map<string, number>();
    for (const o of oportunidades) {
      if (!o.motivoDescarte || o.postulantesDescartados <= 0) continue;
    ...
    const conteoMotivoEntrevista = new Map<string, number>();
    for (const o of oportunidades) {
    ...
    const conteoMotivoNoIngreso = new Map<string, number>();
    for (const o of oportunidades) {
...
const construirFiltrosComercial = (query: Record<string, unknown>): Prisma.OportunidadComercialWhereInput => {
  const where: Prisma.OportunidadComercialWhereInput = {};

  if (query.empresaId) where.empresaId = Number(query.empresaId);
  if (query.ejecutivoId) where.ejecutivoId = Number(query.ejecutivoId);
  if (query.servicio) where.tipoServicio = String(query.servicio);
  if (query.oportunidadId) where.id = Number(query.oportunidadId);
  ...
  if (query.estado) {
    ...
    if (estado === "PERDIDA") {
      ...
      where.etapa = estado as EtapaOportunidadComercial;
      where.perdida = false;
    }
  }
 ...
 */
export const obtenerDashboardComercial = async (req: AuthRequest, res: Response) => {
  try {
    const where = construirFiltrosComercial(req.query as Record<string, unknown>);
    ...
    const empresasProspectadas = new Set(
      oportunidades.filter((o) => o.empresaId !== null).map((o) => o.empresaId as number)
    ).size;

    const tiemposCierre = oportunidadesGanadasArr
      .filter((o) => o.DatosCierreGanada?.fechaCierre)
      .map((o) => diasEntre(o.createdAt, o.DatosCierreGanada!.fechaCierre as Date));
    ...
    const funnelComercial = [
      ...
      { etapa: "PERDIDA" as EtapaOportunidadComercial, cantidad: oportunidadesPerdidasArr.length },
    ];

    type AcumuladorEjecutivo = {
      nombre: string;
      empresas: Set<number>;
      oportunidadesActivas: number;
      ganadas: number;
    ...
    const porEjecutivo = new Map<number, AcumuladorEjecutivo>();
    ...
    for (const o of oportunidades) {
      ...
      if (o.empresaId !== null) grupo.empresas.add(o.empresaId);
      if (o.etapa !== "GANADA" && !o.perdida) grupo.oportunidadesActivas += 1;
      if (o.etapa === "GANADA") grupo.ganadas += 1;
      if (o.perdida) grupo.perdidas += 1;
    ...
    type AcumuladorServicio = { oportunidades: number; ganadas: number; perdidas: number; montoTotal: number };
    const porServicio = new Map<string, AcumuladorServicio>();
    ...
    for (const o of oportunidades) {
      const clave =
        o.tipoServicio === TIPO_SERVICIO_OTRO_DASHBOARD && o.tipoServicioOtro ? o.tipoServicioOtro : o.tipoServicio;
      if (!porServicio.has(clave)) porServicio.set(clave, { oportunidades: 0, ganadas: 0, perdidas: 0, montoTotal: 0 });
    ...
    const conteoMotivoPerdida = new Map<string, number>();
    ...
    const conteoEstadoNegociacion = new Map<EstadoNegociacionComercial, number>();
    for (const estado of ESTADOS_NEGOCIACION_DASHBOARD) conteoEstadoNegociacion.set(estado, 0);
    for (const o of oportunidades) {
